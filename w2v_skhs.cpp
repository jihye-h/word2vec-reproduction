...
public:
    ...
    int negative = 5;
    int layer1_size = 300;
    float alpha = 0.025; // cbow:0.05, skip-gram:0.025
    int get_window();
...
void W2V::TrainOneFile_cbow(int file){
    ...
    //learning rate
    if(file == 0)alpha = 0.05;
    else if (alpha > 0.00005) {
      alpha = 0.05 - file*0.0005;
    }
    else{
      alpha = 0.00005;
    }
...
void W2V::TrainOneFile_skipgram(int file){
    ...
    //learning rate
    if(file == 0)alpha = 0.025;
    else if (alpha > 0.000025) {
      alpha = 0.025 - file*0.00025;
    }
    else{
      alpha = 0.000025;
    }
...
void W2V::SaveVectors(){
  FILE *ft = fopen("trainedvec_skhs_0509","wb");
  FILE *fb = fopen("trainedvecb_skhs_0509", "wb");
  fprintf(ft, "%d %d\n", vocab_size, 300);
  fprintf(fb, "%d %d\n", vocab_size, 300);
  for(int i = 0; i<vocab_size;i++){
